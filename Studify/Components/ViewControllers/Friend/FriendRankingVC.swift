...
class FriendRankingVC: UIViewController {
    ...
    func setDataOnChart(withJSON json: JSON,title: String) {
        ...
        self.chartView.show(viewController: self,title: title)
    }
}
...
extension FriendRankingVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if (indexPath.section == 0) {
            ...
            guard let user = API.shared.currentUser else {return}
            API.shared.study.record(token: user.token) { (json, err) in
                //print(json.debugDescription)
                if (err == nil) {
                    self.setDataOnChart(withJSON: json, title: user.name + "님의 최근 5일")
            ...
            API.shared.study.record(token: API.shared.currentFriends[indexPath.row].token) { (json, err) in
                //print(json.debugDescription)
                if (err == nil) {
                    self.setDataOnChart(withJSON: json, title: "\(API.shared.currentFriends[indexPath.row].name)님의 최근 5일")
                }
            }
        }
    }
